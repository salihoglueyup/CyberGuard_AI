...
export default function AttackTraining() {
    ...
    const TrainingTab = () => (
        <div className="space-y-6">
            ...
            <div className="bg-[var(--hud-surface)]/50 border border-[var(--hud-border)] rounded-xl p-5">
                ...
                <div className="grid grid-cols-3 gap-3">
                    {DATASETS.map(ds => (
                        ...
                        >
                            <h4 className="font-medium text-[var(--hud-text)]">{ds.name}</h4>
                            <p className="text-sm text-[var(--hud-text-muted)]">{ds.size}</p>
                            <div className="flex flex-wrap gap-1 mt-2">
                                {ds.attacks.slice(0, 3).map(a => (
                                    <span key={a} className="text-xs bg-[var(--hud-panel)] px-1.5 py-0.5 rounded">
                                        {a}
                                    </span>
                                ))}
                            </div>
                ...
                <div className="grid grid-cols-3 md:grid-cols-5 gap-2">
                    {ATTACK_TYPES.map(attack => (
                        ...
                        >
                            ...
                            <p className="text-sm text-[var(--hud-text)] mt-1">{attack.name}</p>
            ...
            <div className="bg-[var(--hud-surface)]/50 border border-[var(--hud-border)] rounded-xl p-5">
                <h3 className="text-lg font-semibold text-[var(--hud-text)] mb-4 flex items-center gap-2">
                    <Settings className="w-5 h-5 text-[var(--hud-text-muted)]" />
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <div>
                        <label className="block text-sm text-[var(--hud-text-muted)] mb-1">Patience</label>
                        <input
                            type="number"
                    ...
                    <div>
                        <label className="block text-sm text-[var(--hud-text-muted)] mb-1">Max Samples</label>
                        <input
                            type="number"
                ...
                <div className="flex items-center gap-6 mt-4">
                    <label className="flex items-center gap-2 text-[var(--hud-text-muted)] cursor-pointer">
                        <input
                            type="checkbox"
                ...
                <div className="mt-4">
                    <label className="block text-sm text-[var(--hud-text-muted)] mb-1">Model İsmi (opsiyonel)</label>
                    <input
                        type="text"
                        value={config.model_name}
                        onChange={(e) => setConfig({ ...config, model_name: e.target.value })}
                        placeholder="örn: ddos_detector_v1"
                        className="w-full md:w-1/2 bg-[var(--hud-bg)] border border-[var(--hud-border)] rounded-lg px-3 py-2 text-[var(--hud-text)]"
                    />
                </div>
            </div>
            ...
            <button
                ...
                className="w-full bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-700 hover:to-blue-700 text-[var(--hud-text)] py-3 rounded-xl flex items-center justify-center gap-2 transition-all font-semibold"
            >
            ...
            {trainingSessions.filter(s => s.status !== 'completed' && s.status !== 'failed').length > 0 && (
                <div className="bg-[var(--hud-surface)]/50 border border-[var(--hud-border)] rounded-xl p-5">
                    <h3 className="text-lg font-semibold text-[var(--hud-text)] mb-4">📋 Aktif Eğitimler</h3>
                    {trainingSessions
                        .filter(s => s.status !== 'completed' && s.status !== 'failed')
                        .map(session => (
                            <div key={session.id} className="bg-[var(--hud-bg)]/50 rounded-lg p-4 mb-2">
                                <div className="flex justify-between mb-2">
                                    <span className="text-[var(--hud-text)]">{session.dataset} - {session.attack_types.join(', ')}</span>
                                ...
                                <p className="text-sm text-[var(--hud-text-muted)] mt-1">{session.progress}% tamamlandı</p>
                            </div>
                        ))}
                </div>
            )}
        ...
        <div className="space-y-6">
            ...
            <div className="bg-[var(--hud-surface)]/50 border border-[var(--hud-border)] rounded-xl p-5">
                ...
                <div className="grid grid-cols-3 gap-4 mb-4">
                    ...
                    <div>
                        <label className="block text-sm text-[var(--hud-text-muted)] mb-1">Window Size</label>
                        <input
                            type="number"
                ...
                <div className="flex gap-2">
                    {idsStatus?.status === 'running' ? (
                        <button
                            onClick={stopIDS}
                            className="flex-1 bg-red-600 hover:bg-red-700 text-[var(--hud-text)] py-2 rounded-lg flex items-center justify-center gap-2"
                        >
                        ...
                        <button
                            ...
                            className="flex-1 bg-green-600 hover:bg-green-700 disabled:bg-[var(--hud-panel)] text-[var(--hud-text)] py-2 rounded-lg flex items-center justify-center gap-2"
                        >
            ...
            {idsStatus?.status === 'running' && idsStatus.metrics && (
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div className="bg-[var(--hud-surface)]/50 border border-[var(--hud-border)] rounded-xl p-4">
                        <p className="text-[var(--hud-text-muted)] text-sm">Toplam Paket</p>
                        <p className="text-2xl font-bold text-[var(--hud-text)]">{idsStatus.metrics.total_packets.toLocaleString()}</p>
                    </div>
                    <div className="bg-[var(--hud-surface)]/50 border border-[var(--hud-border)] rounded-xl p-4">
                        <p className="text-[var(--hud-text-muted)] text-sm">Saldırı Tespit</p>
                        <p className="text-2xl font-bold text-red-400">{idsStatus.metrics.attack_packets}</p>
                    </div>
                    <div className="bg-[var(--hud-surface)]/50 border border-[var(--hud-border)] rounded-xl p-4">
                        <p className="text-[var(--hud-text-muted)] text-sm">Paket/Saniye</p>
                        <p className="text-2xl font-bold text-blue-400">{idsStatus.metrics.packets_per_second?.toFixed(1)}</p>
                    </div>
                    <div className="bg-[var(--hud-surface)]/50 border border-[var(--hud-border)] rounded-xl p-4">
                        <p className="text-[var(--hud-text-muted)] text-sm">İşlem Süresi</p>
            ...
            <div className="bg-[var(--hud-surface)]/50 border border-[var(--hud-border)] rounded-xl p-5">
                ...
                <div className="space-y-2 max-h-96 overflow-y-auto">
                    {idsAlerts.length > 0 ? (
                        ...
                        <p className="text-[var(--hud-text-dim)] text-center py-4">Henüz alert yok</p>
                    )}
                </div>
            </div>
        </div>
            ...
            <div className="space-y-3">
                {trainedModels.length > 0 ? (
                    trainedModels.map((model, idx) => (
                        <div key={idx} className="bg-[var(--hud-bg)]/50 rounded-lg p-4 flex items-center justify-between">
                            <div>
                                <h4 className="font-medium text-[var(--hud-text)]">{model.name}</h4>
                                <p className="text-sm text-[var(--hud-text-muted)]">{model.size_mb} MB • {model.created_at}</p>
                            </div>
                            <button
                    ...
                    <p className="text-[var(--hud-text-dim)] text-center py-4">Henüz eğitilmiş model yok</p>
                )}
            </div>
        </div>
    );
        ...
        <div className="p-6 space-y-6">
            ...
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-[var(--hud-text-muted)] text-xs tracking-wide mt-1">DDoS, DoS, Botnet, Probe ve daha fazlası</p>
                </div>
                <button
                    onClick={loadData}
                    className="flex items-center gap-2 px-4 py-2 bg-[var(--hud-panel)] hover:bg-[var(--hud-border)] rounded-lg text-[var(--hud-text)]"
                >
