...
export default function MLModels() {
    ...
    const handleDeploy = async (modelId) => {
        try {
            ...
            if (res.data.success) {
                toast.success('Model başarıyla deploy edildi!');
                loadData();
            } else {
                toast.error(res.data.error);
            }
        } catch (error) {
            toast.error(`Deploy hatası: ${error.message}`);
        }
    };
    ...
    const handleArchive = async (modelId) => {
        try {
            ...
            if (res.data.success) {
                toast.success('Model arşivlendi!');
                loadData();
            } else {
                toast.error(res.data.error);
            }
    ...
    const handleDelete = async (modelId) => {
        if (window.confirm('Bu modeli silmek istediğinizden emin misiniz? Bu işlem geri alınamaz!')) {
            try {
    ...
    const startTraining = async () => {
        try {
            ...
            toast.error(`Training başlatılamadı: ${error.message}`);
            setIsTraining(false);
        }
    };
    ...
    return (
        <div className="space-y-6 fade-in">
            ...
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-xl font-semibold text-[var(--hud-text)]">ML Model Dashboard</h1>
                    <p className="text-[var(--hud-text-muted)] text-xs tracking-wide mt-1">Training, İnceleme, Karşılaştırma, Analiz</p>
                </div>
          ...
          ======================================== */}
            {activeTab === 'overview' && (
                <div className="space-y-6">
                            ...
                            <div className="card">
                                <div className="flex items-center justify-between">
                                    <div>
                                        ...
                                        <p className="text-2xl font-bold text-[var(--hud-text)]">{stats.total_models}</p>
                                    </div>
                            ...
                            <div className="card">
                                <div className="flex items-center justify-between">
                                    <div>
                                        ...
                                        <p className="text-2xl font-bold text-[var(--hud-emerald)]">{stats.deployed}</p>
                                    </div>
                                    <Rocket className="w-8 h-8 text-[var(--hud-emerald)]" />
                                </div>
                            </div>
                            <div className="card">
                                <div className="flex items-center justify-between">
                                    <div>
                                        ...
                                        <p className="text-lg font-bold text-[var(--hud-text)] truncate">{stats.best_model || '-'}</p>
                                    </div>
                            ...
                            <div className="card">
                                <div className="flex items-center justify-between">
                                    <div>
                                        ...
                                        <p className="text-2xl font-bold text-[var(--hud-amber)]">{(stats.best_accuracy * 100).toFixed(1)}%</p>
                                    </div>
                                    <BarChart3 className="w-8 h-8 text-[var(--hud-amber)]" />
                                </div>
                            </div>
                        </div>
                    )}
                    ...
                    {/* Performance Chart */}
                    <div className="card">
                        <h3 className="text-lg font-semibold text-[var(--hud-text)] mb-4">📈 Model Performans Karşılaştırması</h3>
                        <div className="h-80">
                    ...
                    {/* Model List */}
                    <div className="card">
                        <h3 className="text-lg font-semibold text-[var(--hud-text)] mb-4">📋 Model Detayları</h3>
                        <div className="space-y-4">
                            {models.map((model, idx) => (
                                <div key={model.id || idx} className="p-4 bg-slate-800 rounded-lg">
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center gap-3">
                                            <Brain className={`w-6 h-6 ${model.status === 'deployed' ? 'text-[var(--hud-emerald)]' : 'text-slate-400'}`} />
                                            <div>
                                                <p className="text-[var(--hud-text)] font-medium">🤖 {model.name}</p>
                                                <p className="text-slate-400 text-sm">{model.status || 'Unknown'}</p>
                                            </div>
                                        </div>
                                        <span className={`px-2 py-1 text-xs rounded ${model.status === 'deployed' ? 'bg-emerald-500/20 text-[var(--hud-emerald)]' : 'bg-slate-700'
                                            }`}>{model.status}</span>
                                    </div>
            ...
            {activeTab === 'inspect' && (
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <div className="card lg:col-span-1">
                        <h3 className="text-lg font-semibold text-[var(--hud-text)] mb-4">🔍 Model Seç</h3>
                    ...
                    <div className="card lg:col-span-2">
                        {selectedModel ? (
                            <div className="space-y-6">
                                <div className="flex items-center justify-between">
                                    <h3 className="text-xl font-bold text-[var(--hud-text)]">{selectedModel.name}</h3>
                                    <span className={`px-3 py-1 rounded-full text-sm ${selectedModel.status === 'deployed' ? 'bg-emerald-500/20 text-[var(--hud-emerald)]' : 'bg-slate-700'
                                ...
                                <div className="grid grid-cols-4 gap-4">
                                    ...
                                    <div className="bg-slate-800 p-3 rounded-lg text-center">
                                        <p className="text-2xl font-bold text-[var(--hud-emerald)]">{((selectedModel.precision || 0) * 100).toFixed(1)}%</p>
                                        <p className="text-slate-400 text-sm">Precision</p>
                                    </div>
                                    <div className="bg-slate-800 p-3 rounded-lg text-center">
                                        <p className="text-2xl font-bold text-[var(--hud-amber)]">{((selectedModel.recall || 0) * 100).toFixed(1)}%</p>
                                ...
                                <div className="bg-slate-800 p-4 rounded-lg">
                                    <h4 className="font-semibold text-[var(--hud-text)] mb-3">📊 Performans Radar Grafiği</h4>
                                ...
                                <div className="bg-slate-800 p-4 rounded-lg">
                                    <h4 className="font-semibold text-[var(--hud-text)] mb-3">📈 Metrik Karşılaştırması</h4>
                                ...
                                {/* Model Detayları */}
                                <div className="bg-slate-800 p-4 rounded-lg">
                                    <h4 className="font-semibold text-[var(--hud-text)] mb-3">🔧 Model Detayları</h4>
                                    <div className="grid grid-cols-2 gap-4 text-sm">
                                        <div>
                                            <p className="text-slate-400">Model ID: <code className="text-[var(--hud-text)]">{selectedModel.id}</code></p>
                                            <p className="text-slate-400">Framework: <span className="text-[var(--hud-text)]">{selectedModel.framework}</span></p>
                                            <p className="text-slate-400">Model Type: <span className="text-[var(--hud-text)]">{selectedModel.model_type}</span></p>
                                        </div>
                                        <div>
                                            <p className="text-slate-400">Train Samples: <span className="text-[var(--hud-text)]">{selectedModel.train_samples?.toLocaleString()}</span></p>
                                            <p className="text-slate-400">Test Samples: <span className="text-[var(--hud-text)]">{selectedModel.test_samples?.toLocaleString()}</span></p>
                                            <p className="text-slate-400">Epochs: <span className="text-[var(--hud-text)]">{selectedModel.epochs}</span></p>
                                        </div>
                                    </div>
                                </div>

                                <div className="bg-slate-800 p-4 rounded-lg">
                                    <h4 className="font-semibold text-[var(--hud-text)] mb-2">📝 Açıklama</h4>
          ...
          ======================================== */}
            {activeTab === 'predict' && (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <div className="card">
                        <h3 className="text-lg font-semibold text-[var(--hud-text)] mb-4">🎯 Hızlı Tahmin</h3>
                        ...
                        <div className="flex gap-4 mb-6">
                            <div className="flex-1">
                                ...
                                <select
                                    ...
                                    className="w-full px-3 py-2 bg-slate-800 border border-slate-700 rounded-lg text-[var(--hud-text)]"
                                >
                        ...
                        {prediction ? (
                            <div className={`p-6 rounded-xl ${prediction.success
                                ...
                                {prediction.success ? (
                                    <div className="space-y-4">
                                        <div className="flex items-center gap-3">
                                            {prediction.prediction === 'malicious'
                                                ...
                                                : <CheckCircle className="w-12 h-12 text-[var(--hud-emerald)]" />
                                            }
                                            <div>
                                                <span className="text-3xl font-bold text-[var(--hud-text)]">
                                        ...
                                        <div className="grid grid-cols-2 gap-4 mt-4">
                                            <div className="bg-slate-800/50 p-4 rounded-lg text-center">
                                                ...
                                                <p className="text-3xl font-bold text-[var(--hud-text)]">{prediction.confidence_percentage}%</p>
                                            ...
                                            <div className="bg-slate-800/50 p-4 rounded-lg text-center">
                                                ...
                                                <p className={`text-3xl font-bold ${prediction.risk_level === 'CRITICAL' ? 'text-red-400' :
                                                    prediction.risk_level === 'HIGH' ? 'text-orange-400' :
                                                        prediction.risk_level === 'MEDIUM' ? 'text-[var(--hud-amber)]' :
                                                            'text-[var(--hud-emerald)]'
                                                    }`}>
                                        ...
                                        <details className="mt-4">
                                            <summary className="text-slate-400 cursor-pointer hover:text-[var(--hud-text)]">🔍 Detaylı Bilgi</summary>
                                    ...
                                    <p className="text-[var(--hud-amber)]">❌ {prediction.error}</p>
                                )}
                            </div>
                        ) : (
                            <div className="text-center py-12">
            ...
            {activeTab === 'training' && (
                <div className="space-y-6">
                    ...
                    {isTraining && trainingSession && (
                        <div className="card border-2 border-blue-500/50">
                            <div className="flex items-center justify-between mb-4">
                                <h3 className="text-lg font-semibold text-[var(--hud-text)]">
                    ...
                    {trainingSession?.status === 'completed' && !isTraining && (
                        <div className="card border-2 border-emerald-500/50">
                            <div className="flex items-center gap-3 mb-4">
                                <CheckCircle className="w-10 h-10 text-[var(--hud-emerald)]" />
                                <div>
                                    <h3 className="text-xl font-bold text-[var(--hud-text)]">✅ Training Tamamlandı!</h3>
                            ...
                            <div className="grid grid-cols-3 gap-4 mb-4">
                                <div className="bg-slate-800 p-3 rounded-lg text-center">
                                    ...
                                    <p className="text-[var(--hud-text)] font-mono text-sm truncate">{trainingSession.model_id}</p>
                                </div>
                                <div className="bg-slate-800 p-3 rounded-lg text-center">
                                    <p className="text-slate-400 text-sm">Süre</p>
                                    <p className="text-[var(--hud-text)] font-bold">{trainingSession.duration}</p>
                                </div>
                                <div className="bg-slate-800 p-3 rounded-lg text-center">
                                    <p className="text-slate-400 text-sm">Accuracy</p>
                                    <p className="text-[var(--hud-emerald)] font-bold">
                    ...
                    {!isTraining && trainingSession?.status !== 'completed' && (
                        <div className="card">
                            <h3 className="text-lg font-semibold text-[var(--hud-text)] mb-4">🏗️ Yeni Model Eğitimi</h3>
                            ...
                            <div className="grid grid-cols-2 gap-6 mb-6">
                                <div>
                                    <h4 className="font-semibold text-[var(--hud-text)] mb-3">📝 Model Bilgileri</h4>
                                    <div className="space-y-4">
                                        <div>
                                        ...
                                        <div>
                                            ...
                                            <textarea
                                                ...
                                                className="w-full px-3 py-2 bg-slate-800 border border-slate-700 rounded-lg text-[var(--hud-text)] h-20"
                                            />
                                        </div>
                                        <div>
                                ...
                                <div>
                                    <h4 className="font-semibold text-[var(--hud-text)] mb-3">⚙️ Hyperparameters</h4>
                    ...
                    {trainingSessions.length > 0 && (
                        <div className="card">
                            <h3 className="text-lg font-semibold text-[var(--hud-text)] mb-4">📜 Training Geçmişi</h3>
                            <div className="space-y-2">
                                {trainingSessions.slice(0, 5).map((session, i) => (
                                    <div key={i} className="flex items-center justify-between p-3 bg-slate-800 rounded-lg">
                                        <div className="flex items-center gap-3">
                                            {session.status === 'completed' && <CheckCircle className="w-5 h-5 text-[var(--hud-emerald)]" />}
                                            ...
                                            <div>
                                                <p className="text-[var(--hud-text)] font-medium">{session.config?.model_name}</p>
                                                <p className="text-slate-400 text-sm">{session.start_time?.slice(0, 19)}</p>
                                            </div>
                                        </div>
                                        <span className={`px-2 py-1 rounded text-xs ${session.status === 'completed' ? 'bg-emerald-500/20 text-[var(--hud-emerald)]' :
          ...
          ======================================== */}
            {activeTab === 'compare' && (
                <div className="space-y-6">
                    <div className="card">
                        <h3 className="text-lg font-semibold text-[var(--hud-text)] mb-4">📊 Karşılaştırılacak Modelleri Seçin</h3>
                            ...
                            {models.map((model) => (
                                ...
                                >
                                    <input
                                        ...
                                        className="hidden"
                                    />
                                    <p className="font-medium text-[var(--hud-text)]">{model.name}</p>
                                    <p className="text-sm text-slate-400">{((model.accuracy || 0) * 100).toFixed(1)}% acc</p>
                                </label>
                            ))}
                        </div>
                    ...
                    {selectedForCompare.length >= 2 && (
                        <>
                            {/* Metrik Tablosu */}
                            <div className="card">
                                <h3 className="text-lg font-semibold text-[var(--hud-text)] mb-4">📊 Metrik Karşılaştırması</h3>
                                    ...
                                    <table className="w-full text-sm">
                                        ...
                                        <tbody className="divide-y divide-slate-800">
                                            {selectedForCompare.map((m) => (
                                                <tr key={m.id}>
                                                    <td className="py-2 text-[var(--hud-text)] font-medium">{m.name}</td>
                                                    <td className="py-2 text-blue-400">{((m.accuracy || 0) * 100).toFixed(2)}%</td>
                                                    <td className="py-2 text-[var(--hud-emerald)]">{((m.precision || 0) * 100).toFixed(2)}%</td>
                                                    <td className="py-2 text-[var(--hud-amber)]">{((m.recall || 0) * 100).toFixed(2)}%</td>
                                                    <td className="py-2 text-purple-400">{((m.f1_score || 0) * 100).toFixed(2)}%</td>
                                                    <td className="py-2"><span className={`px-2 py-1 rounded text-xs ${m.status === 'deployed' ? 'bg-emerald-500/20 text-[var(--hud-emerald)]' : 'bg-slate-700'
                                                        }`}>{m.status}</span></td>
                                                </tr>
                                            ))}
                                        </tbody>
                            ...
                            {/* Radar Chart */}
                            <div className="card">
                                <h3 className="text-lg font-semibold text-[var(--hud-text)] mb-4">🎯 Radar Chart</h3>
                                <div className="h-96">
                                    <ResponsiveContainer width="100%" height="100%">
                                        <RadarChart data={[
          ...
          ======================================== */}
            {activeTab === 'manage' && (
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <div className="card">
                        <h3 className="text-lg font-semibold text-[var(--hud-text)] mb-4">⚙️ Model Seç</h3>
                        <div className="space-y-2">
                            {models.map((model) => (
                                <div
                                    key={model.id}
