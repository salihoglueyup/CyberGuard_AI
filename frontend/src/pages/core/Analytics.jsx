            ...
            <div className="bg-slate-900/95 backdrop-blur-xl border border-slate-700/50 rounded-xl p-4 shadow-2xl">
                ...
                {payload.map((entry, index) => (
                    <div key={index} className="flex items-center gap-2">
                        ...
                        />
                        <span className="text-[var(--hud-text)] font-medium">{entry.name}:</span>
    ...
    return (
        <div className="relative flex flex-col items-center">
            ...
            <div className="mt-2 text-center">
                <p className="text-2xl font-bold text-[var(--hud-text)]">{Math.round(value)}%</p>
...
export default function Analytics() {
    ...
    const loadInitialData = useCallback(async () => {
        ...
        try {
            ...
            const [statsRes, hourlyRes, typeRes, sevRes, modelsRes] = results;

            if (statsRes.status === 'fulfilled' && statsRes.value?.data?.success)
                setInitialStats(statsRes.value.data.data);

            if (hourlyRes.status === 'fulfilled' && hourlyRes.value?.data?.success) {
                const timeline = hourlyRes.value.data.data || [];
            ...
            if (typeRes.status === 'fulfilled' && typeRes.value?.data?.success) {
                const types = typeRes.value.data.data || {};
            ...
            if (sevRes.status === 'fulfilled' && sevRes.value?.data?.success) {
                setSeverityData(Object.entries(sevRes.value.data.data || {}).map(([name, value]) => ({
                    name,
                    value
                })));
            }
            ...
            if (modelsRes.status === 'fulfilled' && modelsRes.value?.data?.success)
                setModelStats(modelsRes.value.data.data);

            // Log any rejected promises
            results.forEach((r, i) => {
                if (r.status === 'rejected') console.warn(`Analytics API #${i} failed:`, r.reason);
            });
            ...
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-xl font-semibold text-[var(--hud-text)] flex items-center gap-3">
                        <BarChart3 className="w-6 h-6 text-[var(--hud-cyan)]" />
                        Analitik Dashboard
                    </h1>
                ...
                <div className="flex items-center gap-3">
                    <select
                        ...
                        className="bg-slate-800/50 backdrop-blur border border-slate-700/50 rounded-xl px-4 py-2 text-[var(--hud-text)] text-sm focus:border-blue-500 transition-colors"
                    >
                        ...
                        <div className="flex items-center justify-between mb-6">
                            <div className="flex items-center gap-3">
                                <div className="p-2 bg-gradient-to-br from-blue-500 to-purple-500 rounded-lg">
                                    <TrendingUp className="w-4 h-4 text-[var(--hud-text)]" />
                                </div>
                                <h3 className="text-lg font-semibold text-[var(--hud-text)]">Saatlik Trend</h3>
                            </div>
                            {isConnected && (
                        ...
                        <div className="flex items-center justify-between mb-6">
                            <div className="flex items-center gap-3">
                                <div className="p-2 bg-gradient-to-br from-orange-500 to-red-500 rounded-lg">
                                    <AlertTriangle className="w-4 h-4 text-[var(--hud-text)]" />
                                </div>
                                <h3 className="text-lg font-semibold text-[var(--hud-text)]">Saldırı Türleri</h3>
                            </div>
                        </div>
                                    ...
                                    <div className="flex-1 space-y-3">
                                        {typeData.slice(0, 6).map((item, idx) => (
                                            <div key={idx} className="flex items-center justify-between p-2 rounded-lg hover:bg-slate-800/50 transition-colors">
                                                ...
                                                <span className="text-[var(--hud-text)] font-bold">{item.value}%</span>
                                            </div>
                                        ))}
                                    </div>
                                </>
                    ...
                    <Card className="relative bg-slate-900/80 backdrop-blur-xl border-slate-800/50">
                        <div className="flex items-center gap-3 mb-6">
                            <div className="p-2 bg-gradient-to-br from-yellow-500 to-green-500 rounded-lg">
                                <BarChart3 className="w-4 h-4 text-[var(--hud-text)]" />
                            </div>
                            <h3 className="text-lg font-semibold text-[var(--hud-text)]">Şiddet Dağılımı</h3>
                        </div>
                        <div className="h-64">
                        ...
                        <div className="flex items-center justify-between mb-6">
                            <div className="flex items-center gap-3">
                                <div className="p-2 bg-gradient-to-br from-cyan-500 to-blue-500 rounded-lg">
                                    <Server className="w-4 h-4 text-[var(--hud-text)]" />
                                </div>
                                <h3 className="text-lg font-semibold text-[var(--hud-text)]">Sistem</h3>
                            </div>
                            {isConnected && (
                                <span className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></span>
                            )}
