...
export default function Logs() {
    ...
    const clearLogs = async () => {
        if (!window.confirm('Tüm logları silmek istediğinizden emin misiniz?')) return;
    ...
    const columns = [
        ...
        },
        {
            key: 'message',
            label: 'Mesaj',
            render: (val) => <span className="text-[var(--hud-text)]">{val || '-'}</span>
        },
    ];
            ...
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-xl font-semibold text-[var(--hud-text)] flex items-center gap-3">
                        <FileText className="w-6 h-6 text-[var(--hud-cyan)]" />
                        Sistem Logları
                    </h1>
                ...
                <Card className="p-4 cursor-pointer hover:border-blue-500/50 transition-colors" onClick={() => setLevelFilter(levelFilter === 'info' ? 'all' : 'info')}>
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-2xl font-bold text-[var(--hud-text)]">{infoCount}</p>
                        </div>
                        {levelIcons.info}
                    </div>
                </Card>
                ...
                <Card className="p-4 cursor-pointer hover:border-slate-500/50 transition-colors" onClick={() => setLevelFilter(levelFilter === 'debug' ? 'all' : 'debug')}>
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-2xl font-bold text-[var(--hud-text)]">{debugCount}</p>
                        </div>
                        {levelIcons.debug}
                    </div>
                </Card>
