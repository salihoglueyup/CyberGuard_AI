...
export default function Reports() {
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (val, row) => (
                <div>
                    <p className="text-[var(--hud-text)] font-medium">{val}</p>
            ...
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-xl font-semibold text-[var(--hud-text)] flex items-center gap-3">
                        <FileBarChart className="w-6 h-6 text-[var(--hud-cyan)]" />
                        Raporlar
                    </h1>
                    ...
                    <Card key={idx} className="p-4">
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-2xl font-bold text-[var(--hud-text)]">{stat.value}</p>
                            </div>
                        ...
                        {Object.entries(typeLabels).map(([type, label]) => (
                            ...
                            >
                                <div className="flex items-center gap-3">
                                    {typeIcons[type]}
                                    <span className="text-[var(--hud-text)] font-medium">{label}</span>
                                </div>
                                {newReportType === type && (
                ...
                {selectedReport && (
                    <div className="space-y-4">
                        <div className="flex items-center gap-4 p-4 bg-slate-800/50 rounded-xl">
                            ...
                            <div>
                                <p className="font-semibold text-[var(--hud-text)]">{selectedReport.name}</p>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div className="p-3 bg-slate-800/30 rounded-lg">
                                ...
                                <p className="text-[var(--hud-text)] font-medium">{selectedReport.size || '-'}</p>
                            </div>
                            <div className="p-3 bg-slate-800/30 rounded-lg">
                                <p className="text-xs text-slate-400">Oluşturulma Tarihi</p>
                                <p className="text-[var(--hud-text)] font-medium">
                        ...
                        {selectedReport.data && (
                            <div className="p-4 bg-slate-800/30 rounded-lg">
                                ...
                                <pre className="text-sm text-[var(--hud-text)] overflow-auto max-h-40">
