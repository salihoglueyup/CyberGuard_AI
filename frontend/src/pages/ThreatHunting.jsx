...
const ThreatHunting = () => {
    ...
    return (
        <div className="min-h-screen bg-gray-900 text-white p-6">
            <div className="mb-6">
                <h1 className="text-3xl font-bold text-green-400">🔍 Threat Hunting</h1>
                <p className="text-gray-400">Proaktif tehdit arama ve soruşturma</p>
            </div>
                                ...
                                {results.results?.logs?.slice(0, 10).map((r, i) => (
                                    <div key={`log-${i}`} className="p-3 rounded-lg bg-gray-700/50 border-l-4 border-blue-500">
                                        <div className="flex justify-between">
                                            ...
                                            <span className="text-xs text-gray-400">Satır: {r.line_number}</span>
                                        </div>
                        ...
                        <div className="space-y-2">
                            {investigations?.map((inv) => (
                                <div key={inv.id} className="p-3 bg-gray-700/50 rounded-lg">
                                    ...
                                    <p className="text-xs text-gray-400 mt-1">{inv.matches || 0} bulgu • {inv.assignee || 'Atanmadı'}</p>
                                </div>
                            ))}
                            {(!investigations || investigations.length === 0) && (
                                <p className="text-gray-500 text-center py-2">Soruşturma bulunamadı</p>
                            )}
                        </div>
                    </div>
                </div>
