...
const VulnScanner = () => {
    ...
    const getSeverityColor = (severity) => {
        const colors = {
            critical: 'bg-red-500 text-[var(--hud-text)]',
            high: 'bg-orange-500 text-[var(--hud-text)]',
            medium: 'bg-yellow-500 text-black',
            low: 'bg-green-500 text-[var(--hud-text)]'
        };
    ...
    const getRiskBadge = (risk) => {
        ...
        return colors[risk] || 'bg-[var(--hud-panel)]';
    };
    ...
    return (
        <div className="relative min-h-screen bg-[var(--hud-bg)] text-[var(--hud-text)] p-6">
            <div className="max-w-7xl mx-auto">
                {/* Header */}
                <div className="mb-8">
                    <h1 className="text-xl font-semibold text-[var(--hud-text)]">Vulnerability Scanner</h1>
                ...
                <div className="bg-[var(--hud-surface)] rounded-xl p-6 mb-6">
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                        <div>
                            <label className="block text-sm text-[var(--hud-text-muted)] mb-1">Hedef IP/Hostname</label>
                            <input
                                type="text"
                                value={target}
                        ...
                        <div>
                            <label className="block text-sm text-[var(--hud-text-muted)] mb-1">Tarama Tipi</label>
                            <select
                                value={scanType}
                ...
                <div className="bg-[var(--hud-surface)] rounded-xl p-6 mb-6">
                    ...
                    <div className="flex gap-4">
                        <input
                            ...
                            className="flex-1 bg-[var(--hud-panel)] border border-[var(--hud-border)] rounded-lg px-4 py-2"
                    ...
                    {cveResult && (
                        <div className="mt-4 bg-[var(--hud-panel)]/50 rounded-lg p-4">
                            <div className="flex items-center gap-3 mb-2">
                                ...
                                <span className="text-[var(--hud-text-muted)]">CVSS: {cveResult.cvss}</span>
                            </div>
                            <p className="text-[var(--hud-text)]">{cveResult.description}</p>
                        ...
                        <div className="bg-[var(--hud-surface)] rounded-xl p-6">
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                                ...
                                <div className="text-center">
                                    ...
                                    <div className="text-[var(--hud-text-muted)] text-sm">High</div>
                                </div>
                                <div className="text-center">
                                    <div className="text-3xl font-bold text-yellow-400">{scanResult.summary?.medium || 0}</div>
                                    <div className="text-[var(--hud-text-muted)] text-sm">Medium</div>
                                </div>
                                <div className="text-center">
                                    <div className="text-3xl font-bold text-green-400">{scanResult.summary?.low || 0}</div>
                                    <div className="text-[var(--hud-text-muted)] text-sm">Low</div>
                                </div>
                                <div className="text-center">
                                    <div className="text-3xl font-bold text-[var(--hud-cyan)]">{scanResult.open_ports?.length || 0}</div>
                                    <div className="text-[var(--hud-text-muted)] text-sm">Open Ports</div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div className="bg-[var(--hud-surface)] rounded-xl p-6">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                    {scanResult.open_ports.map((port, i) => (
                                        <div key={i} className="bg-[var(--hud-panel)]/50 rounded-lg p-4">
                                            <div className="flex items-center justify-between mb-2">
                                                <span className="font-mono text-xl font-bold text-[var(--hud-cyan)]">:{port.port}</span>
                                                <span className={`px-2 py-1 rounded text-xs ${getRiskBadge(port.risk)}`}>
                                                    {port.risk}
                                                </span>
                                            </div>
                                            <div className="text-[var(--hud-text-muted)] text-sm">{port.service}</div>
                                            <div className="text-[var(--hud-text-dim)] text-xs">{port.version}</div>
                                        </div>
                                    ))}
                                </div>
                            </div>
                                        ...
                                        <div key={i} className="bg-[var(--hud-panel)]/50 rounded-lg p-4">
                                            <div className="flex items-start justify-between">
                                                <div>
                                                    ...
                                                    <p className="text-[var(--hud-text-muted)] text-sm">{vuln.description}</p>
                                                    {vuln.cve_id && (
                                                        <span className="inline-block mt-2 bg-[var(--hud-border)] px-2 py-1 rounded text-xs">
                                                            {vuln.cve_id}
                                                        </span>
                                                    )}
                                                </div>
                                                <div className="text-right">
                                                    ...
                                                    <div className="text-[var(--hud-text-dim)] text-xs">CVSS</div>
                                                </div>
                                            </div>
