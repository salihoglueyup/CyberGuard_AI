...
function HudTooltip({ active, payload, label }) {
  ...
  return (
    <div style={{
      ...
      <p style={{ color: '#4a5568', marginTop: 4, borderTop: '1px solid rgba(56,189,248,0.1)', paddingTop: 4 }}>
        Toplam: <strong>{payload.reduce((s, p) => s + (p.value || 0), 0).toLocaleString()}</strong>
      </p>
    </div>
  );
}

export default function HudStackedArea({
  data = [],
  dataKeys = [],
  xKey = 'name',
  colors = HUD_COLORS,
