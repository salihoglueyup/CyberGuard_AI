...
cache_db = os.path.join(str(PROJECT_ROOT), "data", "geoip_cache.db")
...
def get_from_cache(ip: str) -> dict | None:
...
def lookup_ip(ip: str) -> dict:
    ...
    # Skip private/local/reserved IPs (correct RFC 1918 check)
    if ip_obj.is_private or ip_obj.is_loopback or ip_obj.is_reserved:
        return get_fallback_location("US", ip)

    # Try ip-api.com (HTTP only on free tier; HTTPS requires paid plan)
