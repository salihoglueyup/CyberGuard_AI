...
# Cache for disk size calculation (TTL: 60 seconds)
_disk_size_cache = {"value": 0, "timestamp": 0}
...
async def get_dashboard_summary():
    ...
    try:
        ...
        # Toplam disk kullanımı (cached, TTL 60s)
        if time.time() - _disk_size_cache["timestamp"] > 60:
            total_size = 0
                ...
                for root, dirs, files in os.walk(folder_path):
                    for f in files:
                        try:
                            ...
                            pass
            _disk_size_cache["value"] = total_size
            _disk_size_cache["timestamp"] = time.time()
        else:
            total_size = _disk_size_cache["value"]
        ...
        for a in attacks:
            ...
            if ts:
                try:
                    ...
                    logger.debug(f"Dashboard timestamp parse error: {e}")
