...
async def get_table_data(
    ...
    try:
        ...
        with db.get_connection() as conn:
            ...
            # Veriyi çek - table_name allowed_tables listesinden doğrulandı
            safe_query = f"SELECT * FROM [{table_name}] ORDER BY id DESC LIMIT ? OFFSET ?"
            cursor.execute(safe_query, (limit, offset))
...
@router.post("/query")
async def execute_query(query: SQLQuery, user: dict = Depends(require_auth)):
    ...
    try:
        ...
        # Ek güvenlik: semicolon ile birden fazla statement engelle
        if ";" in sql:
            raise HTTPException(status_code=400, detail="Birden fazla SQL statement desteklenmez")
...
async def get_table_schema(table_name: str, user: dict = Depends(require_auth)):
    """Tablo şemasını getir"""
    try:
        # Güvenlik: İzin verilen tablolar
        allowed_tables = [
            "attacks", "network_logs", "scan_results", "system_logs",
            "chat_history", "system_metrics", "ip_blacklist",
            "file_quarantine", "defences",
        ]
        if table_name not in allowed_tables:
            return {"success": False, "error": f"Tablo bulunamadı: {table_name}", "data": []}
