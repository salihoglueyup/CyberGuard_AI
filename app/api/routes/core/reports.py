...
async def get_report(report_id: str):
    ...
    try:
        meta_path = os.path.join(REPORTS_DIR, f"{report_id}.json")
        if not os.path.abspath(meta_path).startswith(os.path.abspath(REPORTS_DIR)):
            raise HTTPException(status_code=400, detail="Geçersiz rapor yolu")
            ...
            for filename in os.listdir(REPORTS_DIR):
                if filename.endswith(".json"):
                    ...
                    try:
                        ...
                        logger.warning(f"Failed to load report stats {filename}: {e}")
