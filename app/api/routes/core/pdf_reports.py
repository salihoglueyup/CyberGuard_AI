...
<body>
    <h1>🛡️ CyberGuard AI Security Report</h1>
    <p>Generated: {esc(str(data['generated_at']))}</p>
    <p>Period: {esc(str(data['period']))}</p>
    
    <div class="summary">
        <h2>Executive Summary</h2>
        <ul>
            <li>Total Alerts: {int(data['summary']['total_alerts'])}</li>
            <li>Critical Alerts: {int(data['summary']['critical_alerts'])}</li>
            <li>Incidents: {int(data['summary']['incidents'])}</li>
            <li>Threats Blocked: {int(data['summary']['threats_blocked'])}</li>
        </ul>
    </div>
"""
    ...
    for section in data.get("sections", []):
        html_str += f"""
    <div class="section">
        <h3>{esc(str(section['title']))}</h3>
        <p>{esc(str(section['content']))}</p>
    </div>
"""
...
@router.get("/history")
async def get_history(limit: int = 20, user: dict = Depends(require_auth)):
    return {"success": True, "data": {"reports": REPORT_HISTORY[-limit:][::-1]}}


@router.get("/report/{report_id}")
async def get_report(report_id: str, user: dict = Depends(require_auth)):
...
async def download_report(report_id: str, user: dict = Depends(require_auth)):
    filepath = os.path.join(reports_dir, f"{report_id}.html")
