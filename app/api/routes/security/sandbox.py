            ...
            findings.append(
                {
                    ...
                    "severity": "medium",
                    "description": "Contains embedded scripting",
                }
            )
            break
...
@router.get("/status")
async def get_sandbox_status(user: dict = Depends(require_auth)):
    """Get sandbox status"""
    return {
        "success": True,
        "data": {
...
@router.post("/analyze")
async def analyze_file(file: UploadFile = File(...), user: dict = Depends(require_auth)):
    """Analyze uploaded file"""
    # Read file content in chunks to prevent OOM on large uploads
    ...
    while True:
        ...
        if len(content) > max_size:
            raise HTTPException(status_code=413, detail="Dosya boyutu 50MB'ı aşamaz")
...
async def analyze_hash(file_hash: str, user: dict = Depends(require_auth)):
...
@router.get("/analyses")
async def get_analyses(limit: int = 50, user: dict = Depends(require_auth)):
...
@router.get("/recent")
async def get_recent_analyses(user: dict = Depends(require_auth)):
...
@router.get("/stats")
async def get_sandbox_stats(user: dict = Depends(require_auth)):
