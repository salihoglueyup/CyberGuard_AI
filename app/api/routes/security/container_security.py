...
@router.get("/status")
async def get_container_security_status(user: dict = Depends(require_auth)):
...
async def get_containers(user: dict = Depends(require_auth)):
    """Get all containers"""
...
@router.get("/images")
async def get_images(user: dict = Depends(require_auth)):
...
@router.post("/scan")
async def scan_image(request: ContainerScanRequest, user: dict = Depends(require_auth)):
...
@router.get("/scans")
async def get_scan_history(limit: int = 20, user: dict = Depends(require_auth)):
    """Get scan history"""
...
async def get_scan_result(scan_id: str, user: dict = Depends(require_auth)):
...
async def get_all_vulnerabilities(user: dict = Depends(require_auth)):
...
@router.get("/stats")
async def get_container_stats(user: dict = Depends(require_auth)):
