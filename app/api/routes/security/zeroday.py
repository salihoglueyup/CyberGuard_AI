...
def calc_anomaly_score(features, model):
    if model is None:
        ...
        high_z = z[z > 2]
        score = min(1.0, float(np.mean(high_z)) / 5) if len(high_z) > 0 else 0.0
        return {
            "score": score,
            "method": "statistical",
        }
...
@router.post("/analyze")
async def analyze(request: AnalysisRequest, user: dict = Depends(require_auth)):
    model = load_model()
...
async def get_detections(limit: int = 50, user: dict = Depends(require_auth)):
    return {
        "success": True,
...
async def resolve(det_id: str, user: dict = Depends(require_auth)):
    for d in DETECTIONS:
        if d.get("id") == det_id:
            d["resolved"] = True
            save_detections()
