...
@router.get("/attacks")
async def list_attacks(user: dict = Depends(require_auth)):
    return {
        "success": True,
        "data": {
            "attacks": [
...
@router.post("/test")
async def run_adversarial_test(test: AdversarialTest, user: dict = Depends(require_auth)):
...
@router.get("/tests")
async def get_tests(limit: int = 50, user: dict = Depends(require_auth)):
    return {
        "success": True,
...
async def get_test(test_id: str, user: dict = Depends(require_auth)):
    for t in TESTS:
