...
def load_samples():
    ...
    if os.path.exists(gan_file):
        try:
            with open(gan_file) as f:
                GENERATED_SAMPLES = json.load(f)
        except (OSError, json.JSONDecodeError) as e:
            logger.warning(f"Failed to load GAN samples: {e}")
...
async def list_attack_types(user: dict = Depends(require_auth)):
    types = []
...
async def generate_samples(request: GenerateRequest, user: dict = Depends(require_auth)):
...
@router.get("/samples")
async def get_samples(attack_type: str | None = None, limit: int = 100, user: dict = Depends(require_auth)):
...
@router.get("/export")
async def export_samples(attack_type: str | None = None, user: dict = Depends(require_auth)):
