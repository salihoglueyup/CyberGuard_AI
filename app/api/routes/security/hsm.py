"""
HSM (Hardware Security Module) API - REAL DATA VERSION
Simulates HSM operations with file-based key storage
"""

import base64
import hashlib
import json
import logging
import os
import secrets
from datetime import datetime

from cryptography.fernet import Fernet
from fastapi import APIRouter, Depends, HTTPException
from pydantic import BaseModel

from app.api.routes.auth import require_auth

logger = logging.getLogger(__name__)

router = APIRouter()

from app.paths import PROJECT_ROOT

project_root = str(PROJECT_ROOT)
data_dir = os.path.join(project_root, "data")
hsm_file = os.path.join(data_dir, "hsm_keys.json")

HSM_DATA = {"keys": [], "operations": []}


class KeyGenRequest(BaseModel):
    name: str
    key_type: str = "aes256"  # aes256, rsa2048, ecdsa
    purpose: str = "encryption"


class EncryptRequest(BaseModel):
    key_id: str
    plaintext: str


def load_hsm():
    global HSM_DATA
    if os.path.exists(hsm_file):
        try:
            with open(hsm_file) as f:
                HSM_DATA.update(json.load(f))
        except (OSError, json.JSONDecodeError) as e:
            logger.warning(f"Failed to load HSM data: {e}")


def save_hsm():
    os.makedirs(os.path.dirname(hsm_file), exist_ok=True)
    with open(hsm_file, "w") as f:
        json.dump(HSM_DATA, f, indent=2, default=str)
    try:
        os.chmod(hsm_file, 0o600)
    except OSError:
        pass


def generate_key(key_type: str) -> str:
    """Generate a cryptographic key"""
    if key_type == "aes256":
        return base64.b64encode(secrets.token_bytes(32)).decode()
    elif key_type == "rsa2048":
        # Simulated RSA key (in real HSM, actual RSA key would be generated)
        return base64.b64encode(secrets.token_bytes(256)).decode()
    elif key_type == "ecdsa":
        return base64.b64encode(secrets.token_bytes(32)).decode()
    else:
        return base64.b64encode(secrets.token_bytes(32)).decode()


def encrypt_data(key: str, plaintext: str) -> str:
    """AES encryption via Fernet"""
    # Derive a Fernet-compatible key from the stored key material
    key_bytes = base64.b64decode(key)
    fernet_key = base64.urlsafe_b64encode(key_bytes[:32])
    f = Fernet(fernet_key)
    return f.encrypt(plaintext.encode()).decode()


def decrypt_data(key: str, ciphertext: str) -> str:
    """AES decryption via Fernet"""
    key_bytes = base64.b64decode(key)
    fernet_key = base64.urlsafe_b64encode(key_bytes[:32])
    f = Fernet(fernet_key)
    return f.decrypt(ciphertext.encode()).decode()


load_hsm()


@router.get("/status")
async def get_status(user: dict = Depends(require_auth)):
    return {
        "success": True,
        "data": {
            "status": "active",
            "mode": "software_simulation",
            "total_keys": len(HSM_DATA["keys"]),
            "total_operations": len(HSM_DATA["operations"]),
            "supported_algorithms": ["aes256", "rsa2048", "ecdsa"],
        },
    }


@router.post("/keys")
async def generate_hsm_key(request: KeyGenRequest, user: dict = Depends(require_auth)):
    key_material = generate_key(request.key_type)

    new_key = {
        "id": f"KEY-{secrets.token_hex(4).upper()}",
        "name": request.name,
        "type": request.key_type,
        "purpose": request.purpose,
        "key_hash": hashlib.sha256(key_material.encode()).hexdigest()[:16],
        "created_at": datetime.now().isoformat(),
        "status": "active",
    }

    # Store key material in memory only — never persisted to disk
    if not hasattr(generate_hsm_key, '_key_store'):
        generate_hsm_key._key_store = {}
    generate_hsm_key._key_store[new_key["id"]] = key_material

    # Cap key store to prevent unbounded memory growth
    if len(generate_hsm_key._key_store) > 1000:
        oldest_keys = list(generate_hsm_key._key_store.keys())[:-500]
        for k in oldest_keys:
            del generate_hsm_key._key_store[k]

    HSM_DATA["keys"].append(new_key)
    if len(HSM_DATA["keys"]) > 1000:
        HSM_DATA["keys"] = HSM_DATA["keys"][-500:]
    save_hsm()

    # Don't return actual key material in production
    return {
        "success": True,
        "data": {
            "id": new_key["id"],
            "name": new_key["name"],
            "type": new_key["type"],
            "key_hash": new_key["key_hash"],
        },
    }


@router.get("/keys")
async def list_keys(user: dict = Depends(require_auth)):
    # Never return key material
    safe_keys = []
    for k in HSM_DATA["keys"]:
        safe_keys.append(
            {
                "id": k["id"],
                "name": k["name"],
                "type": k["type"],
                "purpose": k["purpose"],
                "status": k["status"],
                "created_at": k["created_at"],
            }
        )
    return {"success": True, "data": {"keys": safe_keys}}


@router.delete("/keys/{key_id}")
async def delete_key(key_id: str, user: dict = Depends(require_auth)):
    for i, k in enumerate(HSM_DATA["keys"]):
        if k.get("id") == key_id:
            HSM_DATA["keys"].pop(i)
            save_hsm()
            return {"success": True, "message": "Key deleted"}
    raise HTTPException(status_code=404)


@router.post("/encrypt")
async def encrypt(request: EncryptRequest, user: dict = Depends(require_auth)):
    key = None
    for k in HSM_DATA["keys"]:
        if k.get("id") == request.key_id and k.get("status") == "active":
            key = k
            break

    if not key:
        raise HTTPException(status_code=404, detail="Key not found")

    key_store = getattr(generate_hsm_key, '_key_store', {})
    key_material = key_store.get(request.key_id)
    if not key_material:
        raise HTTPException(status_code=400, detail="Key material not available (server may have restarted)")

    ciphertext = encrypt_data(key_material, request.plaintext)

    # Log operation
    HSM_DATA["operations"].append(
        {
            "type": "encrypt",
            "key_id": request.key_id,
            "timestamp": datetime.now().isoformat(),
        }
    )
    if len(HSM_DATA["operations"]) > 10000:
        HSM_DATA["operations"] = HSM_DATA["operations"][-5000:]
    save_hsm()

    return {"success": True, "data": {"ciphertext": ciphertext}}


@router.post("/decrypt")
async def decrypt(key_id: str, ciphertext: str, user: dict = Depends(require_auth)):
    key = None
    for k in HSM_DATA["keys"]:
        if k.get("id") == key_id and k.get("status") == "active":
            key = k
            break

    if not key:
        raise HTTPException(status_code=404, detail="Key not found")

    key_store = getattr(generate_hsm_key, '_key_store', {})
    key_material = key_store.get(key_id)
    if not key_material:
        raise HTTPException(status_code=400, detail="Key material not available (server may have restarted)")

    try:
        plaintext = decrypt_data(key_material, ciphertext)
    except Exception:
        raise HTTPException(status_code=400, detail="Decryption failed")

    HSM_DATA["operations"].append(
        {"type": "decrypt", "key_id": key_id, "timestamp": datetime.now().isoformat()}
    )
    if len(HSM_DATA["operations"]) > 10000:
        HSM_DATA["operations"] = HSM_DATA["operations"][-5000:]
    save_hsm()

    return {"success": True, "data": {"plaintext": plaintext}}


@router.get("/operations")
async def get_operations(limit: int = 100, user: dict = Depends(require_auth)):
    return {"success": True, "data": {"operations": HSM_DATA["operations"][-limit:]}}


@router.get("/stats")
async def get_stats(user: dict = Depends(require_auth)):
    ops = HSM_DATA["operations"]
    by_type = {}
    for o in ops:
        t = o.get("type", "unknown")
        by_type[t] = by_type.get(t, 0) + 1

    return {
        "success": True,
        "data": {
            "total_keys": len(HSM_DATA["keys"]),
            "active_keys": len(
                [k for k in HSM_DATA["keys"] if k.get("status") == "active"]
            ),
            "total_operations": len(ops),
            "operations_by_type": by_type,
        },
    }
