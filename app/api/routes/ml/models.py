    ...
    try:
        ...
        for m in models:
            if m.get("id") == model_id or m.get("model_id") == model_id:
                ...
                if model_path:
                    # Path traversal koruması: sadece models_dir altındaki dosyaları sil
                    abs_model_dir = os.path.realpath(os.path.dirname(model_path))
                    abs_models_dir = os.path.realpath(models_dir)
                    if abs_model_dir.startswith(abs_models_dir) and os.path.exists(abs_model_dir):
                        try:
