...
class TrafficData(BaseModel):
    ...
    source_info: str | None = "Unknown"
    target_info: str | None = "Unknown"


class BatchTrafficData(BaseModel):
    """Batch traffic data"""

    samples: list[list[float]] = Field(..., description="List of feature vectors", max_length=1000)
    source_info: str | None = "Unknown"
    target_info: str | None = "Unknown"
...
def get_engine():
    """Get or create AI Decision Engine (thread-safe)"""
    global _ai_engine
...
@router.post("/decide")
async def make_decision(data: TrafficData, user: dict = Depends(require_auth)):
    """
...
async def make_batch_decision(data: BatchTrafficData, user: dict = Depends(require_auth)):
...
async def detect_zero_day(data: TrafficData, user: dict = Depends(require_auth)):
...
async def train_zero_day(request: TrainVAERequest, background_tasks: BackgroundTasks, user: dict = Depends(require_auth)):
...
async def get_zero_day_stats(user: dict = Depends(require_auth)):
...
async def select_model(data: TrafficData, user: dict = Depends(require_auth)):
...
async def get_model_select_stats(user: dict = Depends(require_auth)):
...
async def threshold_decision(request: ThresholdRequest, user: dict = Depends(require_auth)):
...
async def train_rl_agent(request: TrainRLRequest, background_tasks: BackgroundTasks, user: dict = Depends(require_auth)):
...
async def get_threshold_stats(user: dict = Depends(require_auth)):
...
@router.post("/explain")
async def explain_attack(request: ExplanationRequest, user: dict = Depends(require_auth)):
...
async def get_feature_info(user: dict = Depends(require_auth)):
    """Get feature information"""
    try:
        from src.ai_decision.explainer import ATTACK_FEATURE_PATTERNS, CICIDS_FEATURES
...
async def generate_report(request: ReportRequest, user: dict = Depends(require_auth)):
...
async def get_report_history(user: dict = Depends(require_auth)):
...
async def get_report_templates(user: dict = Depends(require_auth)):
...
async def get_engine_stats(user: dict = Depends(require_auth)):
...
async def initialize_engine(background_tasks: BackgroundTasks, user: dict = Depends(require_auth)):
...
async def engine_health(user: dict = Depends(require_auth)):
