...
class ExplainRequest(BaseModel):
    ...
    model_id: str | None = "best_cicids2017"
    features: list[float]
    num_features: int = 10
    method: str = "shap"
...
def _cache_explainer(key, explainer):
    if len(_explainer_cache) >= MAX_CACHE_SIZE:
...
def get_shap_explainer(model, background_data: np.ndarray):
    ...
    try:
        ...
        _cache_explainer(model_id, explainer)
        return explainer
    except Exception as e:
...
@router.get("/status")
async def get_xai_status(user: dict = Depends(require_auth)):
...
@router.post("/explain")
async def explain_prediction(request: ExplainRequest, user: dict = Depends(require_auth)):
    ...
    try:
        ...
        logger.exception("XAI explain error")
        return {"success": False, "error": "Explanation failed"}


@router.post("/lime-explain")
async def lime_explain(request: ExplainRequest, user: dict = Depends(require_auth)):
    ...
    try:
        ...
        logger.exception("LIME explain error")
        return {"success": False, "error": "LIME explanation failed"}
...
async def get_feature_importance(model_id: str, sample_size: int = 100, user: dict = Depends(require_auth)):
    ...
    try:
        ...
        return {"success": False, "error": "Feature importance calculation failed"}


@router.get("/global-importance")
async def get_global_importance(user: dict = Depends(require_auth)):
...
async def get_sample_data(num_samples: int = 5, user: dict = Depends(require_auth)):
...
async def get_explanation_methods(user: dict = Depends(require_auth)):
