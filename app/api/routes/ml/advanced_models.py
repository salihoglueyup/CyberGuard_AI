...
def _evict_completed(cache: dict, max_size: int):
    """Remove oldest completed entries when cache exceeds max size"""
    if len(cache) <= max_size:
        return
    completed = [k for k, v in cache.items() if v.get('status') in ('completed', 'failed')]
    for k in completed[:len(cache) - max_size]:
        del cache[k]
...
@router.get("/models")
async def list_advanced_models(user: dict = Depends(require_auth)):
...
@router.get("/compare")
async def compare_advanced_models(user: dict = Depends(require_auth)):
    """Model performans karşılaştırması"""

    # Comparison results dosyasını oku
    results_file = Path(project_root) / "model_artifacts" / "comparison_results.json"
...
async def train_advanced_model(
    ...
    _evict_completed(active_trainings, MAX_TRAININGS)
...
async def get_training_status(training_id: str, user: dict = Depends(require_auth)):
    """Eğitim durumunu al"""
...
@router.get("/train")
async def list_trainings(user: dict = Depends(require_auth)):
...
async def start_optimization(
    ...
    _evict_completed(active_optimizations, MAX_OPTIMIZATIONS)
...
async def get_optimization_status(opt_id: str, user: dict = Depends(require_auth)):
...
@router.get("/optimize")
async def list_optimizations(user: dict = Depends(require_auth)):
    """Tüm optimizasyonları listele"""
    return {"success": True, "data": list(active_optimizations.values())}


@router.post("/ensemble")
async def create_ensemble(request: EnsembleRequest, user: dict = Depends(require_auth)):
...
async def get_live_metrics(user: dict = Depends(require_auth)):
