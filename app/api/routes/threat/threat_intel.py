...
class ThreatQuery(BaseModel):
    indicator: str
    type: str | None = None  # auto-detect if not provided
...
def load_threat_db():
    ...
    if os.path.exists(threat_db_file):
        try:
            with open(threat_db_file, encoding="utf-8") as f:
                loaded = json.load(f)
...
def save_threat_db():
    ...
    # Trim IOCs list to prevent unbounded growth
    if len(THREAT_DB.get("iocs", [])) > 10000:
        THREAT_DB["iocs"] = THREAT_DB["iocs"][-10000:]
...
def check_abuseipdb(ip: str) -> dict | None:
...
def check_virustotal_hash(file_hash: str) -> dict | None:
...
def check_local_db(indicator: str, ioc_type: str) -> dict | None:
...
@router.get("/iocs")
async def get_iocs(ioc_type: str | None = None, limit: int = 100):
