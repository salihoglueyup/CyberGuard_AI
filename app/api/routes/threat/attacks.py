...
async def get_attacks(
    ...
    limit: int = Query(50, ge=1, le=500),  # Reasonable pagination cap
    hours: int | None = Query(None),  # None = tüm zamanlar
...
@router.get("/stats")
async def get_attack_stats(hours: int | None = Query(None)):
...
@router.get("/by-type")
async def get_attacks_by_type(hours: int | None = Query(None)):
...
async def get_attacks_by_severity(hours: int | None = Query(None)):
    """Severity dağılımı"""
    try:
        attacks = db.get_attacks(hours=hours)
