...
def load_hunts():
    ...
    if os.path.exists(hunts_file):
        try:
            with open(hunts_file, encoding="utf-8") as f:
                HUNTS = json.load(f)
        except Exception:
            pass
...
async def perform_hunt(hunt: HuntQuery):
    ...
    # Limit stored hunt results to prevent unbounded memory growth
    if len(HUNT_RESULTS) > 100:
        oldest_keys = sorted(HUNT_RESULTS.keys())[:len(HUNT_RESULTS) - 100]
