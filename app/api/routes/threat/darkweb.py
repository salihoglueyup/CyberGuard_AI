...
def load_breach_db():
    ...
    if os.path.exists(breach_db_file):
        try:
            with open(breach_db_file, encoding="utf-8") as f:
                BREACH_DB.update(json.load(f))
        except Exception as e:
            logger.warning(f"Breach DB yüklenemedi: {e}")
    ...
    if os.path.exists(monitoring_file):
        try:
            ...
            logger.warning(f"Monitoring dosyası yüklenemedi: {e}")
...
async def add_monitoring_asset(asset: MonitoringAsset):
    ...
    new_asset = {
        "id": hashlib.sha256(f"{asset.type}:{asset.value}".encode()).hexdigest()[:12],
        "type": asset.type,
        "value": asset.value,
