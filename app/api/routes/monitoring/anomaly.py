...
def load_anomalies():
    ...
    if os.path.exists(anomalies_file):
        try:
            ...
            print(f"[anomaly] Failed to load anomalies data: {e}")
...
def statistical_anomaly_score(features: list[float]) -> float:
    ...
    high_z = z_scores[z_scores > 2]
    if len(high_z) == 0:
        return 0.0
    return min(1.0, float(np.mean(high_z)) / 3)
...
@router.post("/detect")
async def detect_anomaly(request: AnomalyDetectRequest, user: dict = Depends(require_auth)):
    model = load_model()
...
async def get_anomalies(limit: int = 100, user: dict = Depends(require_auth)):
    return {
        "success": True,
...
async def get_anomaly(anomaly_id: str, user: dict = Depends(require_auth)):
    for a in ANOMALIES:
