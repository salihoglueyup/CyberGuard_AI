...
def load_config():
    ...
    if os.path.exists(siem_config_file):
        try:
            with open(siem_config_file, encoding="utf-8") as f:
                SIEM_CONFIG.update(json.load(f))
        except (OSError, json.JSONDecodeError) as e:
            logger.warning(f"Failed to load SIEM config: {e}")

    if os.path.exists(siem_events_file):
        try:
            with open(siem_events_file, encoding="utf-8") as f:
                SIEM_EVENTS = json.load(f)
        except (OSError, json.JSONDecodeError) as e:
            logger.warning(f"Failed to load SIEM events: {e}")
