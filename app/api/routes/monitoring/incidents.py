...
async def analyze_and_create_incident(event: ThreatEventRequest):
    """
    LLM Agent ile tehdit analizi yap ve otomatik incident oluştur.
    LLM yapılandırılmamışsa kural tabanlı fallback kullanır.
    """
    try:
        from src.ai_decision.threat_agent import ThreatDecisionAgent
        agent = ThreatDecisionAgent()
        result = await agent.handle_threat(event.model_dump())
